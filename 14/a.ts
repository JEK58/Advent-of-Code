...
while (true) {
  ...
  while (!pathBlocked) {
    if (sandPosition[1] > groundLevel) {
      pathBlocked = true;
    }
    const directions = getDirections(sandPosition);

    if (validPosition(directions[0])) {
      sandPosition = directions[0];
    } else if (validPosition(directions[1])) {
      sandPosition = directions[1];
    } else if (validPosition(directions[2])) {
  ...
  if (sandPosition[1] > groundLevel) break;
  fallenUnits++;
}
...
function getBlockedPositions(paths: Position[][]) {
  ...
  for (const path of paths) {
    for (let i = 0; i < path.length - 1; i++) {
      ...
      for (let x = x1; x <= x2; x++) {
        for (let y = y1; y <= y2; y++) _blocked.add(posToString([x, y]));
      }
    }
  }
  return _blocked;
...
function validPosition(pos: Position) {
  return !blocked.has(posToString(pos));
}
